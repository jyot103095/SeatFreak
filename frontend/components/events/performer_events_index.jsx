...
class PerformerEventsIndex extends React.Component {
  ...
  render() {
    ...
    const eventLinks = this.props.events.map(event => <li key={event.id}><Link to={`/events/${event.id}`}>{event.title}</Link></li>);
        ...
        <header className="main-content-splash">
          <div className="main-content-splash-info">
            <h3 className="main-content-route-info">
              <span><Link to="/" >Home</Link> / <Link to={`/categories/${this.props.performer.category}`} >{this.props.performer.category}</Link> / {this.props.performer.name}</span>
            </h3>
